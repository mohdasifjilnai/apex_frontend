 ...
 **/
  getRequestedResponse(url: string, productModuleName?: string,queryParamsUrl?: string) {
    if (queryParamsUrl) {
...
patchRequestedResponse(url: any, body: any) {
    return this.httpService.patchRequest(url, body)
        .pipe(
            map((response: any) => response),
             catchError((err: HttpErrorResponse) => {
                console.log(err);
 ...
 **/
  ...
  deleteData(url:any, body?: any){
    return this.httpService.deleteRequestWithToken(url,body)
    .pipe(
