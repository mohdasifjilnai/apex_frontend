...
export class VehicleDetailsPopupComponent implements OnInit {
  ...
  ngOnInit(): void {
      ...
      (renewalregistartionnumber: any) => {
        ...
        if (this.registrationNumber?.rb_mmv_id) {
          ...
          sessionStorage.setItem(
            'registrationDetails',
            JSON.stringify(renewalregistartionnumber)
          );
    ...
    this.previousRenewalData = JSON.parse(
  ...
  disablevisually() {
    this.visuallyDisabledFields = this.sharedDataService.disableVisually(
      [
        'vehicle_make',
        'vehicle_model',
        'vehicle_variant',
  ...
  patchVehicleData(data: any) {
    ...
    let allNCBProposalUrl = sessionStorage.getItem('allNCBDataProposal');
    if (allNCBProposalUrl) {
      this.vehicleDetailsForm.patchValue({
        policy_expiry: data?.typeofPreviousPolicy,
      });
    } else {
      this.vehicleDetailsForm.patchValue({
    ...
    let policyDateVal = this.policyExpiredDateObject
      ? new Date(this.policyExpiredDateObject)
      : '';
    this.expiryPolicyGetList(registrationDateObject, '', policyDateVal);
  }
    ...
    if (typeof name != 'object') {
      ...
      this.apiservice
        .getRequestedResponse(
          `${ApiConstants.get_vehicle_mmv()}?product_name=${
  ...
  claimedPolicy(data: any, allData?: any) {
    if (data) {
      ...
      if (this.renewalType == 'renewal') {
        ...
        this.vehicleDetailsForm.get('ncb_discount')?.setValue(ncb_count);
      } else {
        if (allData != null) {
          this.vehicleDetailsForm.patchValue({
  ...
  getExpiringPolicy(
    ...
    if (this.registrationNumber) {
      if (
        ...
        if (!policyExpiryDate) {
          if (
            this.registrationNumber?.previous_policy_exp_date != 'NA' &&
            this.registrationNumber?.previous_policy_exp_date != '' &&
            this.registrationNumber?.previous_policy_exp_date != undefined &&
            this.registrationNumber?.previous_policy_exp_date != null
          ) {
      ...
      if (this.vehicleMMVData) {
        ...
        if (
          this.vehicleMMVValue?.policy_expiry_date != 'Not Sure' &&
          this.vehicleMMVValue?.policy_expiry_date != null
        ) {
          policyExpiryDate = new Date(this.vehicleMMVValue?.policy_expiry_date);
        ...
        ?.subscribe((res) => {
          if (res) {
            ...
            if (ownershipTransfer == 'ownerTransfer') {
              let selectedPreviousPolicy = this.vehicleDetailsForm.value
                ?.policy_expiry
                ? this.vehicleDetailsForm.value?.policy_expiry
                : this.expiryList[0]?.rb_expiring_policy_type_code;
 ...
 */
  expiryPolicyGetList(date: any, modifiedDate: any, policyExpDate?: any) {
    ...
    let renewal = sessionStorage.getItem('renewalType');
        ...
        if (Array.isArray(res)) {
          ...
          if (this.registrationNumber && this.editClick == '') {
            ...
            if (
              ...
              if (
                manufacturedYear >= registrationYear - 4 &&
                manufacturedYear <= registrationYear
              ) {
                this.vehicleDetailsForm.patchValue({
                  manufacture_date: new Date(manufactureDate),
                });
            ...
            this.renderer.addClass(document.body, 'dropdown-focus');
            let allNCBProposalUrl =
              sessionStorage.getItem('allNCBDataProposal');
            if (allNCBProposalUrl) {
              // this.vehicleDetailsForm.patchValue({
              //   policy_expiry: data?.policy_expiry_id_data,
              // });
            } else {
              this.vehicleDetailsForm.patchValue({
            ...
            if (
              ...
              this.renewalMMvList.registration_date = new Date(
                registrationDate
              );
              this.getExpiringPolicy(dateObj);
            }
            ...
            if (
              ...
              this.renewalMMvList.manufacture_date = new Date(manufactureDate);
            }
            ...
            if (this.allValue.quotesRequest.previous_policy_exp_date) {
              ...
              if (!this.vehiclePopupList) {
                ...
                this.renewalMMvList.policy_expiry_date = new Date(
                  reformattedDate
                );
              }
            }
            ...
            sessionStorage.setItem(
              'renewalmmvData',
              JSON.stringify(this.renewalMMvList)
            );
          }
    ...
    if (typeof name != 'object' && name.length >= 2) {
      ...
      this.apiservice
        .getRequestedResponse(
          `${ApiConstants.get_vehicle_mmv()}?product_name=${
            this.vehicleTypeValue
          }&make=${selectedMakeValue}&model=${name}&variant=`
        )
        .subscribe(
          (res) => {
            if (Array.isArray(res) && res.length > 0) {
    ...
    if (typeof name != 'object' && name.length >= 2) {
      ...
      this.apiservice
        .getRequestedResponse(
          `${ApiConstants.get_vehicle_mmv()}?product_name=${
            this.vehicleTypeValue
          }&make=${selectedMakeValue}&model=${selectedModalValue}&variant=${name}`
        )
        .subscribe(
          (res: any) => {
            if (Array.isArray(res) && res.length > 0) {
    ...
    if (typeof name != 'object' && name.length >= 2) {
      ...
      this.apiservice
        .getRequestedResponse(
          `${ApiConstants.get_vehicle_mmv()}?product_name=${
            this.vehicleTypeValue
          }&make=${this.modelSelected}&model=${name}&variant=`
        )
        .subscribe(
          (res) => {
            if (Array.isArray(res) && res.length > 0) {
