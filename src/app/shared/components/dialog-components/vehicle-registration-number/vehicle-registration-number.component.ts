...
export class VehicleRegistrationNumberComponent implements OnInit {
  ...
  vehcileRegistrationForm() {
    this.vehicleRegistrationNumberForm = this.formBuild.group({
      ...
      engine_number: [''],
      chassis_number: ['', [Validators.minLength(6), Validators.maxLength(25)]],
    });
  }
            ...
            if (!res?.is_commercial) {
              if (
                (this.quotes_data?.vehicle_type == 'private_car' &&
                  res?.is_four_wheeler) ||
                (this.quotes_data?.vehicle_type == 'two_wheeler' &&
                  res?.is_two_wheeler)
              ) {
                ...
                sessionStorage.setItem(
                  'quotes_data',
                  JSON.stringify(this.quotes_data)
                );
  ...
  addHyphen(event: any) {
    ...
    if (
      this.vehicleRegistrationNumberForm.controls[
